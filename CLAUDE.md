...
**CI has run once and got as far as step 4.** Steps 1 to 3 pass in `debian:trixie-slim`; step 4 failed for a missing host compiler (see below), which is fixed but not yet re-run. Steps 4 to 6 have therefore still only ever completed on macOS.
