...
**Steps 5b and 5c are new and have never been run — not on macOS, not in CI.** They were written against LLVM 23.1.0's own CMake options and clang's driver sources (both read, not recalled), and each asserts its result rather than assuming it, but nothing has compiled yet. Treat a failure there as expected rather than surprising, and read `build/runtimes/build.log` or `build/libobjc2/build.log` — CI uploads both. Verified in advance, and worth not re-deriving: libobjc2 2.3's archive unpacks to `libobjc2-2.3/`, its digest is committed, it has no submodules, and Apple clang 21 emits aarch64 ELF for `aarch64-unknown-linux-musl`, so `HOST_CLANG` needs no override on macOS.
...
- **`python3` is in the `apt-get` list for LLVM's sake, not this repo's**, `clang` is there for step 5c alone (libobjc2 will not be built by anything else), and `gcc g++` are there for step 4's host tree — `debian:trixie-slim` ships no compiler, and steps 1 to 3 do not need one, so the gap only surfaces four steps in. `cmake`/`ninja` are new to SepiaOS, so this install list is longer than either sibling's; **keep it identical in both workflows**, or a release builds in a container CI never tested. Build tools still go in before `actions/checkout`, or checkout silently degrades to a tarball download.
...
- **The runtimes are built by the cross GCC, not by a clang.** This looks wrong and is not. A clang bootstrap has a chicken-and-egg step: CMake's compiler check links a test program, which needs the builtins the build has not produced yet. GCC brings its own libgcc, links on the first try, and is the same compiler/sysroot/flags step 5 already proved. libc++ supports being built by GCC. What the *shipped* clang defaults to is decided in step 5, not by whoever compiled the runtime.
